...
- Add `[` and `]` motions to select the current parent or its next sibling, falling back to the current node's previous or next sibling only when there is no parent, without changing collapse state. [#4](https://github.com/commandzero/tless/pull/4)
