...
impl<'a> Parser<'a> {
    ...
    pub fn scanner_visits(&self) -> usize { self.scanner.visits() }
    ...
    fn enter_container(&mut self) -> ToonResult<()> {
        if self.container_depth == 256 {
            return Err(self.profile_error(crate::types::ProfileErrorKind::UnsafeNesting, "Maximum container depth of 256 exceeded"));
        }
        self.container_depth += 1;
        Ok(())
    }

    fn in_container(&mut self, parse: impl FnOnce(&mut Self) -> ToonResult<Value>) -> ToonResult<Value> {
    ...
    fn parse_value_with_depth(&mut self, depth: usize) -> ToonResult<Value> {
        ...
        match &self.current_token {
            ...
            Token::Number(_) => {
                let next_char_is_colon = matches!(self.scanner.peek(), Some(':'));
                if next_char_is_colon {
                    let key = self.scanner.last_token_text().to_owned();
                    self.advance()?;
                    self.parse_object_with_initial_key(key, depth)
                } else {
                    let first_text = self.scanner.last_token_text().to_string();
                    ...
                    match &self.current_token {
                        ...
                        _ => {
                            crate::utils::number::checked_json_number(&first_text).map(Value::Number)
                        }
                    }
                }
            }
    ...
    fn parse_object(&mut self, depth: usize) -> ToonResult<Value> {
        self.in_container(|parser| parser.parse_object_inner(depth))
    }

    fn parse_object_inner(&mut self, depth: usize) -> ToonResult<Value> {
        validate_depth(depth, MAX_DEPTH)?;
        ...
        loop {
            ...
            let key = match &self.current_token {
                Token::String(s, was_quoted) => {
                    ...
                    if *was_quoted && s.contains('.') && self.options.expand_paths != crate::types::PathExpansionMode::Off {
            ...
            if obj.contains_key(&key) {
                return Err(self.profile_error(crate::types::ProfileErrorKind::DuplicateKey, key.clone()));
            }
    ...
    fn parse_object_with_initial_key(&mut self, key: String, depth: usize) -> ToonResult<Value> {
        self.in_container(|parser| parser.parse_object_with_initial_key_inner(key, depth))
    }

    fn parse_object_with_initial_key_inner(&mut self, key: String, depth: usize) -> ToonResult<Value> {
        validate_depth(depth, MAX_DEPTH)?;
    ...
    fn parse_field_value(&mut self, depth: usize) -> ToonResult<Value> {
        ...
        if matches!(self.current_token, Token::Newline | Token::Eof) {
            ...
            if has_children {
                ...
                self.in_container(|_| Ok(Value::Object(Map::new())))
            }
            ...
            let result = if rest.is_empty() && space_count == 0 {
                ...
                match &self.current_token {
                    ...
                    Token::Number(_) => {
                        let value = crate::utils::number::checked_json_number(&token_text).map(Value::Number);
                        value
                    }
                ...
                match token {
                    ...
                    Token::UnsignedInteger(i) => Ok(serde_json::Number::from(i).into()),
                    Token::Number(_) => {
                        let value = crate::utils::number::checked_json_number(&value_str).map(Value::Number);
                        value
                    }
                    Token::Bool(b) => Ok(Value::Bool(b)),
                    Token::Null => Ok(Value::Null),
                ...
                match &self.current_token {
                    Token::String(s, _) => {
                        if fields.contains(s) {
                            return Err(self.profile_error(crate::types::ProfileErrorKind::DuplicateField, s.clone()));
                        }
                        fields.push(s.clone());
                        self.advance()?;
    ...
    fn parse_array_with_context(
        ...
        self.in_container(|parser| parser.parse_array_with_context_inner(depth, context))
    }

    fn parse_array_with_context_inner(
        &mut self,
        depth: usize,
        context: ArrayParseContext,
    ) -> ToonResult<Value> {
    ...
    fn parse_tabular_array(
        ...
        for row_index in 0..length {
            if matches!(self.current_token, Token::Eof) {
                if self.options.strict {
                    return Err(self.profile_error(crate::types::ProfileErrorKind::CountMismatch, format!(
                            ...
                            if should_parse_more_fields {
                                while !matches!(self.current_token, Token::Eof) {
                                    ...
                                    if obj.contains_key(&field_key) {
                                        return Err(self.profile_error(crate::types::ProfileErrorKind::DuplicateKey, field_key.clone()));
                                    }
                                    self.advance()?;
    ...
    fn parse_primitive(&mut self) -> ToonResult<Value> {
        match &self.current_token {
            ...
            Token::Number(_) => {
                let value = crate::utils::number::checked_json_number(self.scanner.last_token_text()).map(Value::Number);
                self.advance()?;
                value
            }
            Token::String(s, _) => {
    ...
    fn profile_error(&self, kind: crate::types::ProfileErrorKind, message: impl Into<String>) -> ToonError {
        let (line, column) = self.scanner.current_position();
        ToonError::ProfileError { kind, line: Some(line), column: Some(column), message: message.into() }
    }
