...
impl Scanner {
    ...
    #[cfg(debug_assertions)]
    pub fn visits(&self) -> usize { self.visits.get() }

    fn char_at(&self, position: usize) -> Option<&char> {
        #[cfg(debug_assertions)]
        self.visits.set(self.visits.get() + 1);
        self.input.get(position)
    }
    ...
    pub fn peek(&self) -> Option<char> {
        self.char_at(self.position).copied()
    }
    ...
    pub fn peek_ahead(&self, offset: usize) -> Option<char> {
        self.char_at(self.position + offset).copied()
    }

    pub fn advance(&mut self) -> Option<char> {
        if let Some(ch) = self.char_at(self.position).copied() {
            self.position += 1;
            if ch == '\n' {
                self.line += 1;
                self.column = 1;
    ...
    fn parse_number(&self, s: &str) -> ToonResult<Token> {
        ...
        } else if crate::utils::number::is_number_literal(s) {
            // Preserve the token's numeric classification. The parser checks
            // the original lexeme before accepting it as a value.
            Ok(Token::Number(0.0))
        } else {
            Ok(Token::String(s.to_string(), false))
        }
    ...
    pub fn parse_value_string(&self, s: &str) -> ToonResult<Token> {
        ...
        if crate::utils::number::is_number_literal(trimmed) {
            Ok(Token::Number(0.0))
        } else {
